...
def api_quiz_start():
    ...
    # TODO: When wiring up OpenAI, use content/url to generate questions
    # For now, return mock data
    session['current_question'] = 0
    session['article_url'] = url
    session['article_content'] = content
    session['article_title'] = title

    response = {**MOCK_QUESTIONS[0], 'total_questions': len(MOCK_QUESTIONS)}
...
def api_quiz_next():
    ...
    if current >= len(MOCK_QUESTIONS):
        return jsonify({"done": True, "message": "Quiz complete!"})

    return jsonify(MOCK_QUESTIONS[current])
